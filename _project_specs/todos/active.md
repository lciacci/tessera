...
## Handoff — pick up here (2026-08-19. **NINE REVIEW ROUNDS ACROSS TWO BRANCHES, 42 FINDINGS, AND FIVE OF THEM WERE IN THE PREVIOUS ROUND'S FIX.** Two PRs merged. **P3's remediation hint had been ACTIVELY MISLEADING for 24 days** — it said *"check the goal field first"*, true on 2026-07-26 and false from 07-28, and it aimed two sessions at a field that is the largest in **0 of 55** over-budget checkpoints. Rewriting it took three tries: an imperative, then a population claim ("the driver is active_constraints, every other field stays flat" — measured false twice), then a per-payload computation that was honest and *still* pointed at goal. **The branch now COMPUTES**: it names the three largest fields of the payload in front of it and whether dropping the actual driver clears the budget. A hint that must stay true as a distribution moves will rot; one that measures its own instance cannot. **ITEM 13'S PREMISE WAS FALSE AND I NEARLY EXECUTED IT.** It said the T2 bars were met — "24 receipts across 3 projects" — having counted tessera's own 17, which P16's docstring and the stopping rule exclude in the same words. P16, run: 7/10 across 2/3, day 21/30. **The correct fact was on the quietest channel in the repo — a non-fired predicate, invisible unless you ask — and the wrong one rode SessionStart into every session.** Principle #17 pointing the wrong way. **THE REVIEW ANCHOR (item 8) TOOK FIVE ROUNDS and was silently broken after every one of the first four** — inert for its dominant case, then defeatable two more ways, then still carrying round 1's defect on the path its own contract calls primary, then a report branch that was DEAD CODE because two round-4 fixes each correct alone cancelled it between them. That is conclave's F-004 — *the fix is the riskiest, least-reviewed part of a change* — scored against the mechanism written to report F-004; the observatory carries the table. **SIX DISTINGUISHABLE WAYS A GUARD WAS DECORATION, five of them mine**: vacuous by construction; a fixture exercising the variant that works; a guard sharing its seam with the implementation; green through the WRONG guard; a re-plant that never applied; and a test calling the unit while the defect lived in the wire. **One guard failed three of those in sequence before it fired.** Also: `scripts/review/stamp.py` had never had a test, and writing its first suite found two defects review had not. Fleet propagated to all six downstreams; howler's 9 dead declines pruned.)
...
> **`### Next —` MUST precede the first `## ` below it, and `### Standing patterns` MUST stay
> INSIDE this block.** The surfacer's awk exits on the next top-level `## `; the patterns surfacer
> sets `blk=1` on this heading and searches within it. Authoring a new section orphaned that block
> twice before (6 `degraded` events). **This section and the one before it were each written as a
...
### Next — 2026-08-19. Items 8 and 11 CLOSED; 13 rewritten because its premise was FALSE.
Everything still open needs a DECISION except item 1, which is measured and waiting on a fork,
and item 6, which waits on a date. **Item 1 (P3) is the only red predicate and it fires most
sessions.**
...
1. **P3 — the live one. OPTION 2 IS DONE (2026-08-19); the fork between 1 and 3 is open.**
   ...
   **Do NOT re-propose capping goals or capping constraints by recency — both are decided.** And
   ...
   exclusion. The cap bounds the COUNT; that filter bounds the SIZE.
   **Governs:** ADR-0015 · `scripts/mnemos/checkpoint.py` · `docs/contracts/restore-receipt.md`.
2. **The decision surface's SORT.** The truncation notice says what was dropped; it does not
   ...
   **Governs:** ADR-0024.
3. **The decision surface indexes NOTHING in any downstream** — 0 keys in all six. Distributed,
   importable, inert everywhere, with nothing reporting it. Two causes, and fixing the first
   alone buys nothing: the ADR-title regex wants a colon and settempo uses an em dash, **and no
   downstream ADR backticks a repo path at all.** Ship-or-prune is the fork. **Biggest live gap.**
   **Governs:** ADR-0003 · ADR-0006.
4. **Nine live ADR review cadences, not four**, and a purpose-written parser missed one on
   punctuation. The cheap remedy is a required FIELD FORMAT, not a `P17` predicate. Lorenzo's
   call.
   **Governs:** `docs/design-principles.md` principle #16 · `docs/adr/README.md`.
5. **NEW 2026-08-19 — `tessera-sync-harness` prints a permanent unactionable line on all six
   downstreams.** Every downstream pre-commit hook is byte-identical to every OTHER downstream's
   and differs from tessera's by 107 lines for one reason: tessera's runs `scripts/doccheck.py`,
   which is not distributed. So the tool correctly refuses to touch it and correctly cannot match
   it to a tessera version — and says *"differ and match NO tessera version — review by hand"*
   forever. **That is the same ignorable-report shape as the dead declines pruned today, one
   branch over in the same tool.** The question is whether it should distinguish DIVERGED from
   DIFFERENT-BY-CONSTRUCTION; a scaffold-variant marker is the obvious shape and is not obviously
   worth it. (settempo has a second, `.sqlfluff`, legitimately its own under ADR-0012.)
   **Governs:** ADR-0004 · `bin/tessera-sync-harness`.
6. **T2 — NOT readable yet, and the previous version of this item said the opposite.** P16 owns
   the bar and reads **7/10 receipts across 2/3 downstream projects**; tessera's own 17 do not
   count, because they are the confounded ones. **Do not adjudicate before P16 fires** — either
   arm: >=10 across >=3 projects, or the 30-day arm on **2026-08-28**, whose own message says THE
   FINDING IS THE RATE, NOT MNEMOS.
   **What IS established, measured 2026-08-19:** the two downstreams that have filed receipts are
   2,515b (arbiter) and 4,524b (conclave) against a ~10,000-character spill boundary, so their
   `insufficient` verdicts are **not** the P3 confound — the two halves are separable. Downstream
   names `decisions` in **6 of 7** receipts; conclave's checkpoint now carries the field and its
   08-14 and 08-15 receipts still name it missing. **4 of the 6 downstreams have no `decisions`
   key at all**, on checkpoints last written 07-08 to 07-23, so only two are worked and the
   3-project arm may not be reachable by accrual.
   **Governs:** ADR-0015 · `bin/tessera-watch` · `docs/contracts/restore-receipt.md`.
7. **ADR-0005's readiness re-assessment is formally due**, deferred by ADR-0025 on purpose.
   **NOT ACTIONABLE YET:** both ADR-0005 and ADR-0025 name the cadence as **2026-10-09**. Pull it
   forward deliberately or leave it; it is not overdue.
   **Governs:** ADR-0005 · ADR-0025 · ADR-0006.
...
- ADR-0029 (spend guard retired) — https://claude.ai/code/artifact/9ae5bdbd-56fb-45f9-971a-6458ff6de738
  Private artifact. **It is a SNAPSHOT.** The file in `docs/adr/` is the source of truth; if the
  ADR's `Status:` or `Executed:` line moves, this copy silently disagrees. Same shape as
  `docs/promo/index.html` — the class ADR-0026 built the doccheck WARN tier for.

**Two ceilings recorded rather than fixed, both in `bare-python3-hook-scripts-are-probed`:** a
target held in a shell VARIABLE is not resolved — `scripts/doccheck.py` is covered only because a
human listed it; and on a hook whose `-c` body is shell-interpolated the scan falls back to the
WHOLE FILE, so the precision scoping is inert on the two live files that motivated it.
...
### 2026-08-15 — what shipped, and the four guards that were decoration

**THE ONE THING TO KNOW.** Every guard written this session passed on first run, and four of
them were verifying nothing. The pattern is identical each time and it is *not* carelessness
about testing — each was tested, and tested against a break **adjacent to** the code rather
than **in** it:

| Guard | Tested against | Actually broken by | Found by |
|---|---|---|---|
| `decision-surface-honors-path-exemptions` v1 | a new foreign path in an ADR | deleting the filter call | re-plant |
| the same, v2 | filter call, degraded import | **stubbing `_is_exempt` itself** | **review** |
| two mnemos test files | nothing — pytest-style `def test_` | run-tests.sh invokes `python -m` | me, before commit |
| the same two, v2 | — | hand-kept `__main__` list | **review** |

**The rule that would have caught all four, stated once:** *a guard must not share its
predicate — or its runner, or its invocation path — with the thing it guards.* Sharing the
DATA is single-sourcing. Sharing the COMPARISON makes the check an echo. This is standing
pattern #10 sharpened: not merely "run it against the broken state", but **run it against a
break IN the code under test, not beside it.**

1. **`be47c49` — ADR-0023's four review findings.** Title corrected to §6's own conservative
   form (*"only impossible **from a hook**"*, not *"in-harness"* — the latter asserts claim 2,
   the untested one), propagated to the index and promo page. Foreign paths repo-prefixed;
   `hooks/` prefix added so the ADR is visible on the file it prescribes changes to (verified
   before and after: `decision_surface.py hooks/tier-classify-hook` returned ADR-0002 alone,
   now returns both — the amendment edge could NOT rescue it, ADR-0002 has 8 revisiting
   records against a cap of 4 and this ADR sorted last). `~2.5 months` → `~2 months`.
   **Recorded in the ADR rather than cleaned up quietly:** an earlier draft of that
   correction bullet named the five foreign paths in bare backticks and re-introduced all
   five index entries *while describing the bug*.

2. **`696b86b`, reworked by `7367c3d` — decision_surface honours foreign paths.** Phantom
   index keys 14 → 7. See the header for the three defects the first version shipped with.
   The foreign set now lives in `scripts/repo_paths.py` answering ONE question ("another
   repo's or a downstream's?"), unioned back into `PATH_ALLOWLIST` so the data has a single
   definition. New check `decision-surface-deps-ship-downstream` **parses** the imports
   rather than matching a list, so a future module-scope dependency cannot be forgotten in
   both the scaffold and the check at once.

3. **`696b86b` + `3717333` — the mid-word truncation, both halves.** `_truncate_preview` cuts
   on a word boundary and marks the cut; `_render_goals` puts one goal per line. **The 200-char
   cap is UNCHANGED and should stay that way** — it is a privacy decision, it is the only
   limiter still working when the regex redactor fails open, and it feeds a checkpoint payload
   that has already overflowed its delivery channel once. Raising it would not have fixed the
   bug and makes the goal-field problem worse.

4. **`7367c3d` — reading beat grepping, measurably.** Told to read `decision_surface.py`
   rather than grep it, and the read caught a broken intermediate state: the signature change
   had left two call sites passing three arguments and two names unimported. **The module
...
1. **`2eccb2a` — POST-for-fulfilled dropped from the checkpoint render.** The split is the schema's
   own (`.claude/skills/icpg/SKILL.md`): a postcondition is *"what must be true when fulfilled"*, an
   invariant *"what must remain true throughout AND AFTER"*. **`docs/contracts/` carries no iCPG
   contract at all**, so this encodes a reading of that schema rather than contradicting a written
   one — worth knowing before the next person assumes a contract exists.
   **What is NOT claimed: a fulfilled POST is not dead.** `drift.py:92` skips only
   `rejected`/`abandoned`, so these are still evaluated by the decision-drift dimension. Exclusion
   from one render, not retirement — and the note in the payload says so, because the stronger
   sentence would have been false. Only `fulfilled` drops; **`drifted` is kept deliberately** — it
   means the predicate is FAILING, the one state where a POST most needs to be visible.
2. **`cf25330` — `write_checkpoint` measures itself at write time.** The instrument
   `bin/tessera-watch:56-59` already named. P3 structurally *cannot* warn early: it runs at
   SessionStart, so it reports a spill in the same breath as the truncated block, and a payload that
   goes over mid-session is evaluated by nothing until the next one. Warn-only on stderr — **a size
   guard that can refuse to save state is a worse failure than the one it guards.**
3. **THE FINDING, and it is the reason item 1 is not closed: the fix refilled inside the session
   that shipped it.** 7,780b → **8,556b** on this session's own intent. The 24 non-static invariants
   are 23 distinct bodies — no redundancy, all genuinely standing. **A cap there is lossy by
   construction and picks winners by recency, on the payload's most valuable field.** That is why
   the third option was declined rather than deferred: measurement killed it, not scheduling.
4. **`STATIC_PREDICATE` names one family and the class has two** — `test_exists("path")` leaks
   through a filter written for `file_exists("path")` (#11, in the filter itself). **Not widened,
   and the ordering is the point:** the note's justification is *"asserted by something stronger"*,
   and for `test_exists` that is **false today** — deleting the named test file fails neither
   doccheck nor the suite. **Add the doccheck assertion that makes the claim true, THEN widen.**
5. **Two defects of my own, both of a class this repo has already paid for.** The omission note
   first cited **`icpg show <id>`, which has never existed** — a silence *with directions*, worse
   than a silent drop; now guarded by a test that parses commands out of the rendered note and
   executes them. And the new doccheck's **unguarded `read_text()` crashed all 45 checks** under a
   synthetic ROOT — the exact class fixed across `tessera-watch` the day before, re-added by me one
   day later. *Reading that the class exists is not the same as not committing it.*
...
### Next — the live queue is 2026-08-15's; items 1/2/3/5 of the 08-10 queue are CLOSED

**These items are TOP-LEVEL and unindented on purpose.** `tessera-watch-surface.sh` extracts
`/^[0-9]+\. /` inside the first handoff block — a blockquoted list is invisible to it, and an
earlier draft of this very section put them behind `> ` so SessionStart surfaced the SPENT
08-10 queue instead. Caught by running the surfacer, not by reading it.

1. **Read `scripts/repo_paths.py`'s module docstring before touching the decision surface.**
   It carries the two errors that cost the most on 2026-08-15 — reusing `PATH_ALLOWLIST` as
...
3. **FOUR ADR cadences ride on human recall, and registering one changed nothing.**
   **`docs/adr/0011-sqlfluff-evaluation.md` is Status: Watching with `Next check: 2026-09-19`
   ...
   graduation predicate earns it"*. A `P17` over structured state is scoped in the observatory
   and **deliberately unbuilt** — the counter-argument, that three dates over four months is a
   calendar problem rather than a mechanism problem, is real and the call is Lorenzo's.
4. **The checkpoint's goal SELECTION is parked, and the criterion is not yet implementable.**
   Lorenzo, 2026-08-15: historical goals seem **useless unless explicitly revisited or invoked
   during the session**. Sharper than "keep N" or "drop all" — and Mnemos records no signal for
   *use within a session*, so **building that signal is the first task, not changing the
   filter.** Selection is also the payload budget adjudicated 2026-08-10; do not reopen it
   casually. The **presentation** half shipped (`3717333`) and made the problem MORE visible,
...
### 2026-08-10 queue — SPENT, kept as the trail

*(Heading deliberately avoids the words the surfacer keys on, so this spent list is not
re-surfaced alongside the live one above.)*

> ## ▶ 2026-08-10's START HERE (spent — kept as the trail): ITEM 3, THEN ITEM 5. ITEM 1 IS DONE.
>
...
> read whole-file, adjudicated, falsified, and reviewed twice more (2026-08-09 evening). **Nothing
> is in front of item 1 any more.**
>
> **RE-MEASURED at the very end of 2026-08-09 — every figure below replaces an earlier one that was
> already stale when written.** *(Superseded: 11,253b total / 7,205b constraints / INV 16 · 2,406b /
> POST 33 · 4,611b / cut to 6,642b. Kept visible rather than overwritten — the figures drifting
> inside a day is itself the argument for fixing this.)*
>
> | | count | bytes | share |
> |---|---|---|---|
> | **checkpoint total** | | **12,234b** | vs an **8,000b** budget |
> | `active_constraints` | 54 | 7,811b | **63.8%** |
> | ├ `INV` | 18 | 2,667b | 21.8% |
> | └ `POST` | **35** | **4,848b** | **39.6%** |
>
> **AND THE DECIDING FACT, WHICH IS NEW: all 30 iCPG intents are `fulfilled`. ZERO are executing.**
> So *every* `POST` in that checkpoint belongs to an intent that is already closed. A
> **postcondition describes the change** — once its intent is fulfilled it is history, and listing 35
> of them under *"Active Constraints (DO NOT VIOLATE)"* is not merely wasteful, it is **wrong**. An
> **invariant is a standing property** that often outlives the intent (`the offer/receipt split
> holds`) — which is why the cut is `POST`-for-fulfilled, not "constraints are too big".
>
> **Dropping them lands it under budget on its own: 12,234 − 4,848 = 7,386b**, without touching one
> standing invariant.
>
> **The checkpoint GREW 765b during the session that measured it** (11,469b → 12,234b), because that
> session authored two more intents and fulfilled both. The prediction — *"generated by the agent
> itself, and uncapped"* — is now corroborated a fourth time, by the act of writing this entry.
>
> **Mechanism of the leak (unchanged, already located):** `bridge-icpg` mints ConstraintNodes at
> intent creation, they are never-evict, and nothing removes them at fulfilment.
> `scripts/mnemos/checkpoint.py:383` *does* read `status='executing'` — but only for the
> `icpg_state` field, never for constraint selection.
>
> **Two cautions, carried forward — they are constraints on HOW, no longer reasons to defer.**
> (1) This deletes from a never-evict store — ADR-0007 territory. The `INV`/`POST` split is a
> *reading* of the semantics, not a documented contract; **check `docs/contracts/` before encoding
> it**, and prefer the part-1 pattern of a visible omission count over a silent drop. That pattern
> is already live and working — the 64 omitted `file_exists` invariants announce themselves inside
> `active_constraints`, so copy it rather than invent one. (2) It is not a patch: do it on fresh
> context, at the START of a session, not bolted onto the end of one.
>
> **AND THE REASON NOBODY SAW IT COMING IS ITS OWN FINDING — P3 CANNOT CATCH THIS.** `tessera-watch`
> is wired to **SessionStart only** (sole registration in `.claude/settings.json`), while the
> checkpoint is rewritten *during* the session — this one was last written at 13:15, hours in. So P3
> evaluated the morning's file, the checkpoint went over budget mid-session, and nothing
> re-evaluated it; it surfaced only because the surfacer was run by hand. **Worse: when P3 does fire
> at the next SessionStart it is reporting on the checkpoint being delivered at that same moment —
> the warning arrives simultaneously with the harm, never before it.** A predicate guarding
> deliverability can only ever report a spill that already happened. The fix is not another
> predicate: **`write_checkpoint` should measure itself at write time**, which is the same
...
3. **~~ADR-0020's fixture matrix~~ SHIPPED 2026-08-10.** 29 cases, 7 lucky-correct **pairs**, scored
   ...
   distinguish one** — qwen 7/7 pairs, `regex_match` 0/7. The sharp number is that **all 7 of the
   regex's true positives are lucky members — none earned by meaning** (rows: P=0.54 R=0.54
   acc=0.50). *A first draft of this line said "12 of 17 rows, ~70%, row-level accuracy would have
   called it competent" — the denominator omitted the 7 foils. Corrected; 17 turns out to be exactly
   the no-foil denominator, so 70% is what this matrix reports if you never build the twins.*
   **CAVEAT, measured: the same classifier scores P=0.32/R=0.53 on the 114-turn
   silver set and 100% here, so the matrix is a CONTRAST instrument, not a calibration** — do not
   read 7/7 as readiness. Fixtures are **tracked** (`scripts/mnemos/fixtures/`), a deliberate
   deviation from §152's "add them to the silver-label set", which is gitignored; recorded in the
   ADR's `Executed:` line and the fixtures README. Deterministic half runs in `tessera-test`; live
   half is `eval_correction.py --fixtures`. Two side-findings logged, not chased: the observatory
   entry on artifact-counting has its trigger answered, and **Python's bytecode cache can silently
   defeat the re-plant discipline** on same-length plants (its own entry).
4. **REDUCED TO ONE THING (2026-08-10): go do real work in conclave.** ~~howler missing the entire
   spend guard (14 files)~~ **CLOSED — howler does not need it.** It is an Android/iOS app: no GPU,
   no cloud provisioning, no spend-committing commands, so a deny-by-default gate on
   PreToolUse(Bash) guards nothing. *The recorded reason had always been TIMING ("mid-iOS-ship"),
   never need — a reason that expires and then leaves the item on lists forever. It took a human
   re-reading this line to close it; see the F-003 entry, which names that failure mode two bullets
   from where it committed it.* **The fleet is now fully current.**
   What remains is **downstream T2 work — and the instruction is `do real work, not T2 work`.**
   The T2 restore receipt only produces evidence when a session is genuinely under load, so the
   task is to work in conclave on conclave's own problems and let the receipts fall out.
   **Why it cannot be done here:** every receipt so far is a *tessera* receipt, and the standing
   caveat binds — orientation in this repo comes from `active.md` and the standing-patterns block,
   which **a downstream app does not have**. So tessera receipts cannot generalise, and conclave's
   are the only ones that can. *(The other two clauses were never tasks: "don't wire iCPG downstream
   yet" is a standing constraint, and "caching closed except quarry" is a status note.)*
5. **The `bin/` control surface review — BATCH 1 OF 3 DONE (2026-08-10).** ~~the other 19 files,
   ...
   **Batching is by adjudication load, not cost** — the whole job is ~$2.70, but 19 files at once
   ...
   >
   > ~~**NEXT SESSION SHOULD BE `bin/tessera-watch`, WHOLE-FILE** … **Read the reporter before
   > acting on its report.** Hold item 1's P3 part 3 for the session after.~~ **DONE, AND THIS
   > ORDERING CLAIM IS NOW DISCHARGED — it is what unblocks item 1.** `tessera-watch` owned P3's
   > predicate and had never been read; it has now been read whole-file, adjudicated, falsified,
   > and reviewed twice more. P3's predicate was examined directly and found sound (its budget and
   > proxy caveats are stated in its own docstring). **So the instrument is no longer the unknown —
   > act on its report. Item 1 is first.**
   >
...
> ### ⚠ ITEM 7's MITIGATION WAS FALSE, AND IT IS WHY THOSE DEFECTS SAT AT LOW PRIORITY.
>
> The 2026-08-07 handoff said of the unguarded `read_text()` in `p16_t2_receipts`: *"this fails
> LOUD rather than silent — it is a robustness bug, not a silent-failure one,"* because
> `tessera-watch-surface.sh` emits `degraded --reason runner-crashed` on a non-zero exit.
>
> **Measured 2026-08-09: an unhandled Python exception exits 1, and the surfacer reads 1 as
> "something fired."** A session received `=== OBSERVATORY WATCH ===` followed by **nothing**, and
> **no degraded event was written**. It was a silent-failure bug wearing a fire's clothing —
> standing pattern #12 — and the sentence that reclassified it is what kept it at the bottom of
> the list for two days.
>
> The reasoning was sound and the premise was never checked. `rc=1` genuinely does mean "fired";
...
1. **ADR-0021 — deep-agents `v0.7`, verdict REJECT.** Not Watching: there is no condition under
   which Tessera adopts a library that supplies an agent loop, and a Watching status with no
   reachable adoption path is a fiction that generates work forever. The first draft said Watching
   by anchoring on ADR-0020, whose target was a format we could plausibly have used; that premise
   did not transfer. **Adopted, idea-only:** base input tokens as a *tracked metric*. **Recorded
   without a trigger:** `dcode`'s hook contract is a near-clone (same events plus
   `PostToolUseFailure`/`SubagentStart|Stop`, `CLAUDE_PROJECT_DIR` unchanged), so "Claude Code only"
   is a claim about the Mnemos half, not the hooks half. **Third independent convergence** on
   enforcing boundaries at the tool layer rather than model self-policing.
   *Their headline does not survive its own footnote: reward CIs span zero for every model, so what
   is demonstrated is the cost drop, not the absence of harm. This is NOT licence to trim our prefix.*
2. **`scripts/prefix_meter.py` + doccheck `eager-prefix-figure-is-current`.** The ~15.6k figure was a
   one-shot chars/4 estimate frozen in prose on 2026-07-30. **Metered 15,497 tracked; 15,762
   comparable against ~15,600 — flat.** The meter's first act CONFIRMED the observatory entry. What
...
3. **FIVE review passes, none of the findings mine.** *(This item said "ten defects, three passes"
   ...
   exit-0 green. Then arbiter: a **module-level `import prefix_meter` that killed all 41 checks**, so
   the check's own try/except was unreachable. Standing pattern #1 in the code written to guard
   against it.
4. **`referenced-paths-exist` was RED on a clean clone** — docs name `.claude/skills/...`, a
   gitignored symlink. The pre-commit gate blocked on any fresh clone. Fixed by resolving the three
   mirror dirs to their tracked sources; **the narrowness is load-bearing** and guarded both ways.
5. **The `.claude` dogfood symlinks had NO OWNER.** Hand-made 2026-06-24; `install.sh` never touched
   them. Now created idempotently, asserted by `verify()` (machine state) and by doccheck's
   `mirror-links-are-symlinks` (repo state, absence green). **Framework-only** — downstreams get
   skills via the global union (ADR-0009) and have no `skills/` to point at; none of the six has
   these and none should.
6. **Claude Code's loader MEASURED** (fixture + nested `claude -p`). Anchored, **inline**, and
   **through a symlink** all load; absent loads nothing. Confirms the widened regex, which had been
   fixed on inference. And corrects "silently": **the literal `@path` line survives in EVERY case,
   resolved or not**, so its presence carries no information — you cannot tell a live import from a
   dead one by reading `CLAUDE.md`.
7. **Both arbiter-reported `tessera-watch` defects fixed**, plus the class beneath them.
   `evaluate()` now isolates each predicate (six other unguarded reads existed — fixing only P16
   would have been fixing the row); a crash is marked, rendered, **excluded from the fire-log**, and
   turned into **exit 2**, which is the code the surfacer already treats as a crash. `__main__` maps
   *any* unhandled failure to 2, closing the render/append_log/argparse half. P16's read is guarded;
...
- **The observatory's mirror-divergence attribution was wrong.** It named `tessera-sync-skills` /
  ADR-0010 as owner. Those own `~/.claude/skills` — the **global** mirror, a real copy. The
  project-local symlink is a different object with no owner. Corrected in place.
- **"Found independently twice" was over-weighted.** `tessera-verify` and arbiter both flagged the
  literal-vs-symlink branch, and that was treated as corroboration. **Two static readers noting the
...
and both citations were wrong. A wrong citation is worse than silence because it reads as sourced.
Note what has no mechanical subject here — "did the author open the file they cited" is unobservable,
so this is a human re-read, not a check (#3's corollary). **What actually caught it every time was
the adversarial layer**, which is built on the assumption that assertions outrun reading. That
assumption is correct and load-bearing.
## Handoff — 2026-08-07 (cross-repo cleanup after the conclave/arbiter reconciliation. The observatory's three-project entry was the most stale artifact in the system and it seeds an unwritten ADR; conclave F-002 disposed; the Pattern lane renamed pr-arbiter → arbiter with arbiter notified; D1 and D2 closed as dead open-decisions; two live `bin/tessera-watch` defects relayed in by hand from arbiter, item 7. The durable lesson: a lane going stale has no mechanical subject, so no check was built for it — and that is the finding.)
...
- **`busy_timeout` on `ICPGStore._conn()`** — the one review finding deliberately
  SKIPPED. A concurrent reader during a pre-existing graph's first connection can
  hit `database is locked`. Needs a real concurrent workload to size; guessing a
  value here would set a knob from nothing.
- **63 unresolved drift events**, all from this session's own edits. `changed` fires
  on every commit to a tracked file. Resolving per-commit makes `icpg drift` a chore
  people learn to skip — which is how the 700-event backlog formed. The real
  question is whether `changed` should auto-resolve under an active intent.
- **Q1 — does the agent author intent in practice?** Now *askable* (it was
  unanswerable: nothing recorded). Every intent authored today was about building
  the loop, which is the weakest possible evidence for it. Real work elsewhere is
  the test.
- **pr-arbiter has no any-repo entry point** — every harness evaluates its own fixed
  20-PR corpus, and `eval/review_pr.py` is hardcoded to its own repo. Tracked there,
  not here.

---

## Handoff — 2026-07-27 (EARLIER session: four safety mechanisms were silently dead — the spend backstop at 47 against a cap of 3, `tessera-authorize grant` callable by the agent, the SessionStart reporters unable to report their own runner dying, and iCPG's detector scoring the emptiness of its own graph. All four fixed and guarded. ADR-0016 and ADR-0014 decided; review is now Claude-only by decision, not by drift)
