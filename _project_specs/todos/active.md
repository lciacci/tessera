...
> budget**, and `active_constraints` is **7,205b — 64%**, the same share it held before part 1
> shipped. The `file_exists` invariants really are gone (2 of 50 remain); what refilled the space is
> **the five iCPG intents authored during this session**, each carrying 3–5 `INV`/`POST` predicates
> written as prose sentences. Not `file_exists`, so the filter does not see them; never-evict, so
> nothing drops them; **generated by the agent itself, and uncapped.** Part 1 removed one class and a
> different class grew into the gap within three days. The next session's checkpoint WILL spill —
> orient from this file, not from Mnemos.
>
> **THE CUT IS PRINCIPLED, NOT A BYTE LIMIT — measured, so it does not need re-deriving:**
>
> | | count | bytes |
> |---|---|---|
> | `INV` | 16 | 2,406 |
> | `POST` | 33 | **4,611** |
>
> **`POST` is 41% of the whole checkpoint, and dropping it for FULFILLED intents alone takes
> 11,253 → 6,642b, under budget, without touching one standing constraint.** The semantics carry the
> cut: a **postcondition describes the change** — once its intent is fulfilled it is history, and
> listing it under *"Active Constraints (DO NOT VIOLATE)"* is simply wrong. An **invariant is a
> standing property** that often outlives the intent (`the offer/receipt split holds`). **Mechanism
> of the leak:** `bridge-icpg` mints ConstraintNodes at intent creation, they are never-evict, and
> nothing removes them at fulfilment. `scripts/mnemos/checkpoint.py:383` *does* read
> `status='executing'` — but only for the `icpg_state` field, never for constraint selection.
>
> **Two cautions before building it, and they are why it was NOT built on 2026-08-09.** (1) This
> deletes from a never-evict store — ADR-0007 territory. The `INV`/`POST` split is a *reading* of
> the semantics, not a documented contract; check `docs/contracts/` before encoding it, and prefer
> the part-1 pattern of a visible omission count over a silent drop. (2) It is item 1 part 3, which
> this list already says is **not a patch**, and it was surfaced at the end of a long session —
> exactly the shape of change that produced the defects five review passes found that day.
>
...
1. **P3 part 3** — the `write_checkpoint` defect (no `decisions` field in the schema, empty
   `current_subgoal`/`working_memory`, corrupted `progress`). **This session's receipt read
   `insufficient` on `goal`, corroborating the prediction a third time**: orientation came from
   `progress` (commit subjects) and re-reading files, never from the checkpoint. **Part 4** —
   `delivered_chars` in `scripts/restore/offer.py`; still *do it when the hook is next touched for
   another reason*.
