...
> budget**, and `active_constraints` is **7,305b — 64%**, the same share it held before part 1
> shipped. The `file_exists` invariants really are gone; what refilled the space is **the five iCPG
> intents authored during this session**, each carrying 3–5 `INV`/`POST` predicates written as prose
> sentences. Not `file_exists`, so the filter does not see them; never-evict, so nothing drops them;
> **generated by the agent itself, and uncapped.** Part 1 removed one class and a different class
> grew into the gap within three days. A cap or a per-intent budget is the shape of the fix, and the
> next session's checkpoint WILL spill — orient from this file, not from Mnemos.
