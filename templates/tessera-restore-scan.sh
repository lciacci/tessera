#!/bin/bash
# tessera-restore-scan — Stop-hook backstop for the restore receipt (T2, ADR-0015).
#
# Structurally identical to tessera-gate-scan.sh, including its cwd anchoring and its
# spec-11 degraded reporting, because it has the same job: make a TURN happen that the
# model would otherwise skip. Kept as a sibling rather than merged — they answer to
# different contracts and a shared wrapper would couple their failure modes.
#
# The obligation it enforces was written by a DIFFERENT party (mnemos-session-start.sh
# logs `restore_offered`), so an unanswered offer is a detected miss rather than an
# absence of evidence. That split is the whole point: `restore_injected` was one party
# certifying its own delivery, and it said "delivered" four times while the model got
# nothing.
...
# Claude Code Stop hook: restore-scan backstop (principle #17).
#
...
if ! command -v jq >/dev/null 2>&1; then
  printf '%s' "$HOOK_INPUT" | degraded --component restore-scan --reason jq-unavailable \
    --detail "jq is not on PATH; restore-scan cannot parse its own hook input"
  exit 0
fi
...
if [ -z "$TRANSCRIPT_PATH" ]; then
  degraded --component restore-scan --reason no-transcript-path --session "$SESSION_ID" \
...
SCAN="$PROJECT_DIR/scripts/restore/scan.py"
if [ ! -f "$SCAN" ]; then
  degraded --component restore-scan --reason scanner-missing --session "$SESSION_ID" \
...
if ! command -v python3 >/dev/null 2>&1; then
  degraded --component restore-scan --reason no-python3 --session "$SESSION_ID" \
...
if ! cd "$PROJECT_DIR" 2>/dev/null; then
  degraded --component restore-scan --reason cwd-unreachable --session "$SESSION_ID" \
