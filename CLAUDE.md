...
- **`tessera-chaos`** — spec 11's fail-open probes: break a component on purpose, assert Tessera reports it within one session without a human asking. **Expected RED** until `tessera-degraded` + `tessera-watch` P13 ship — that is the spec's ordering, not a defect (a detector certified with itself is a mirror, not an instrument). Deliberately NOT in `tessera-test`, so the main suite's green stays a usable signal while these are legitimately red; doccheck's `chaos-suite-is-reachable` is what stops it rotting unrun. Fold into `run-tests.sh` when the probes go green.
