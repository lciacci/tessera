...
- **Second gap found in the same check: `fatigue.json` is all `None`.** The statusline hook is not writing token metrics this session, so Mnemos's fatigue model (token-util 0.40 weight) and its auto-checkpoint-at-0.60 are **not functioning**. The fatigue subsystem is dark, independent of compaction.
