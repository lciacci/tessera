...
- **Status:** Watching
- **When to revisit:** When the gate-event producer is built (a suggestion-gate hook emitting `suggestion_gate` events per the canonical contract). At that point #12 becomes measurable — and the `should_fire` ground-truth open question in the contract gets its first real data. Close this entry then.
- **Open question it feeds:** does a convention-only gate need a producer at all, or is "Claude surfaces decisions per CLAUDE.md" sufficient? The producer earns its keep only if we want to *measure* gate quality (precision/recall the dashboard already computes). If we never look at that panel on real data, the convention may be the whole feature.
