...
Open GSD is a mature, MIT-licensed framework with a notable provenance worth being explicit about. The original framework (gsd-build/get-shit-done by TÂCHES) appears compromised or abandoned: per GitHub discussion #109, no contact with the original maintainer since 2026-04-01 (~7 weeks of silence as of late May), TÂCHES social accounts deleted or unreachable, and the associated $GSD crypto token publicly linked to a rug-pull. The current open-gsd/gsd-core is a community-maintained continuation that took over via a bit-for-bit mirror and continued development. The current state under open-gsd is substantial: ~3,749 commits on the `next` branch, 23 releases, 2.4k stars on gsd-core. The org also maintains gsd-pi (CLI), gsd-workbench (desktop, coming soon), gsd-cloud (hosted, coming soon), and gsd-browser (Chrome verification). Recent releases (v1.42.1 introduced the Package Legitimacy Gate; v1.40 introduced two-stage hierarchical routing; May 2026 introduced the Minimal Install Profile reducing system-prompt overhead from ~12k to ~700 tokens) demonstrate active feature development. Maintainer model: open-source community with active contributors plus a Discord. Bias-risk assessment, revised: the original framework's ecosystem demonstrated real abandonment risk, and the current open-gsd continuation is a community fork addressing that abandonment. Two readings — reassuring (open source proved resilient) and concerning (the original framework's continuation is less battle-tested than its history alone suggests). This strengthens rather than weakens the chosen verdict ("adopt patterns, keep Tessera as host"): patterns survive framework discontinuity in a way that framework-dependence does not. The downstream products (workbench, cloud) still suggest a future commercial angle but the core is MIT and documentation is unusually thorough (Diataxis-style: tutorials, how-to, reference, explanation).
