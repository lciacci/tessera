...
- **Date:** 2026-07-14
- **Status:** Accepted
- **Executed:** 2026-07-16 de-eagering shipped (`CLAUDE.md`, `skills/security/SKILL.md` kept on-demand); **2026-07-27 the review-skill cut is DONE** — removed: `skills/tessera-code-review/`. It sat unexecuted for thirteen days, during which a session acted on this ADR's verdict while missing the later deferral, and the leftover skill shadowed Claude Code's native `/code-review`. That gap is why the `Executed:` field exists at all; this is the entry it was created to be able to close. Resolved by ADR-0014 (option D — review is Claude-only), which is where ADR-0008 deferred it.
