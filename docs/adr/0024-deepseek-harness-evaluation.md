...
- **Date:** 2026-08-17
- **Status:** Watching
- **Executed:** 2026-08-17 — both adopted concepts. The truncation notice: `scripts/decision_surface.py` (`render_truncation`, `lookup_split`), `scripts/test_decision_surface.py`. Justified exemption entries: `scripts/repo_paths.py` (every entry `{path: reason}`, plus `ABSENT_TESSERA_PATHS`), enforced by `scripts/doccheck.py`'s `absent-index-paths-are-declared`, which also fails on a declaration whose path has come back. That second half closed live queue item 4 in the same change — Braintrust's two `.mdx` paths had been indexed as Tessera paths since 2026-08-06. Corrected from `not yet` by review the same day: the notice landed in the commit that carried this ADR, so `_execution_warning` was rendering *"NOT EXECUTED — decided, never built. Do not act on it as settled."* onto `scripts/repo_paths.py`, telling an editing agent that a half-shipped decision was never begun. That is ADR-0008's 12-day gap inverted, injected as a directive.
